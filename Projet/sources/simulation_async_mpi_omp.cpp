...
void simulation(bool affiche, int* nargs, char** argv[])
{
    ...
    int globRank;
    MPI_Comm_rank(globComm, &globRank);
    ...
    int rank;
    MPI_Comm_split(MPI_COMM_WORLD, colour, globRank , &subComm);
    MPI_Comm_rank(subComm, &rank);

    int nbp_local;
    MPI_Comm_size(subComm, &nbp_local);
    
    std::cout << "RANK / COLOUR / GLOBAL RANK: " << rank << "," << colour << "," << globRank << std::endl;
        ...
        // Chaque processus gère une grille plus petite que l'on assemblera avec les autres
        contexte.taux_population/=nbp_local;
        ...
        std::cout << "Début boucle épidémie" << std::endl << std::flush;
            ...
            épidémie::Grille grosse_grille = épidémie::Grille(contexte.taux_population * nbp_local);
    ...
    while (!quitting)
    {
        ...
        if(colour==0)
        {
            ...
            for ( const auto& e : events)
            {
                if (e->kind_of_event() == sdl2::event::quit)    
                {
                    ...
                    //MPI_Isend(&quitting ,1, MPI_CXX_BOOL,1,1, subComm, &send_req); 
                } 
            }
        }
        ...
        {   
            ...
            //MPI_Irecv(&quitting ,1 , MPI_CXX_BOOL,0,1, subComm, &recv_req); // reçoit l'instruction de quitter, ou non  
            ...
            {
                // attention : fausse les résultats
                //#pragma omp parallel for
            ...
            MPI_Gather(grille.getStatistiques().data(), grille.getStatistiques().size(), MPI_INT, grosse_grille.getStatistiques().data(), grille.getStatistiques().size()*nbp_local, MPI_INT, 0, subComm );
            // Mise à jour des statistiques pour les cases de la grille :
            if(rank==0)
                màjStatistique(grosse_grille, population);
            MPI_Scatter(grosse_grille.getStatistiques().data(),grosse_grille.getStatistiques().size()/nbp_local, MPI_INT, grille.getStatistiques().data(), grille.getStatistiques().size(), MPI_INT, 0, subComm );
        ...
        if(rank==0){
            ...
            send_buff=grosse_grille.getStatistiques();
            //Allgather
            ...
            MPI_Isend(&jours_écoulés ,1, MPI_INT,0,0, globComm, &data_send_req); // envoi de la date
            MPI_Isend(send_buff.data() , 100000, MPI_INT, 0, 0, globComm, &data_send_req); // envoi des données
        ...
        else if (colour==0) 
        {
            ...
            while(!flag){
                ...
                MPI_Iprobe(1, 0, globComm, &flag, &stat);
            }

            //La communication est globale
            MPI_Recv(&nb_jours,1,MPI_INT,1,0,globComm,&data_req); // reçoit la date
            MPI_Recv(statistiques.data(), 100000, MPI_INT, 1, 0, globComm, &data_req); // reçoit les données
            if(affiche){
