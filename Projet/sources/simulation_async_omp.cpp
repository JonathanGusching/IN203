...
void simulation(bool affiche, int* nargs, char** argv[])
{
    ...
    while (!quitting)
    {
        ...
        {   
            ...
            {
                // Si l'on veut garder les mêmes résultats, on ne peut pas paralléliser ici (car cela fait changer la seed)
            ...
            // Si l'on veut garder les mêmes résultats, on ne peut pas paralléliser ici (car cela fait changer la seed), comme précédemment
            //# pragma omp parallel for schedule(static) shared(jours_écoulés, contexte, grille, grippe, agent) reduction(+: compteur_agent, compteur_grippe, mouru)
            ...
            MPI_Isend(send_buff.data() ,3*largeur_grille*hauteur_grille, MPI_INT,0,0, globComm,&data_send_req); // envoi des données
        ...
        else if (rank==0) 
        {
            ...
            MPI_Recv(statistiques.data(),3*largeur_grille*hauteur_grille,MPI_INT,1,0,globComm,&data_req); // reçoit les données
            //RECEPTION
            if(affiche)
